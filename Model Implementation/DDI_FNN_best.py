...
def DNN(X, Y, Epochs, batchsize, layernum=1, verbose=False):
    ...
    test_log = pd.DataFrame(X_test)
    test_log['y'] = pd.Series(class_predictions)
    test_log_counts = test_log['y'].value_counts()
...
final_model,final_f1,final_acc = DNN(red_feat_x, red_feat_y, ep, batch, layer, verbose=True)
...
f1_path = "dnn_f1_best.parquet"
acc_path = "dnn_acc_best.parquet"
...
filename = 'dnn_model_red_best.pkl'
with open(filename, 'wb') as file:
    pickle.dump(dnn_model, file)

dnn_model.save('dnn_model_red_best.keras')