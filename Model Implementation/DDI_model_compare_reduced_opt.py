...
def opt_hps(func, X, Y, epochs, batchsizes, layers=[1], show_progress=False):
    best_epochs = 0
    best_batchsize = 0
    best_acc = 0
    best_layers = 0
    #acc_grid = pd.DataFrame(columns=epochs, index=batchsizes)
    #auc_grid = pd.DataFrame(columns=epochs, index=batchsizes)
    f1_grid = pd.DataFrame(columns=epochs, index=batchsizes)
    #cat_acc_grid = pd.DataFrame(columns=epochs, index=batchsizes)

    iter = 0
    max_rep = len(epochs) * len(batchsizes) * len(layers)

    best_model, best_f1 = func(X, Y, epochs[0], batchsizes[0], layernum=3)
    ...
    for e in range(0,len(epochs)):
        for b in range(0,len(batchsizes)):
            ...
            #aucs = [0] * len(layers)
            f1s = [0] * len(layers)
            #cat_accs = [0] * len(layers)

            for l in range(0,len(layers)):
                if show_progress:
                    iter += 1
                    pct = int(round(iter / max_rep * 100,0))
                    print(f"Performing {epochs[e]} epochs on batches of {batchsizes[b]} with {layers[l]} hidden layers, {pct} percent complete")

                model,  f1_score = func(X, Y, epochs[e], batchsizes[b], layers[l])

                #accs[l] = round(acc,4)
                #aucs[l] = round(AUC,4)
                f1s[l] = round(f1_score,4)
                #cat_accs[l] = round(cat_acc,4)
                ...
                if f1_score > best_f1:
                    ...
                    best_layers = layers[l]
                    best_f1 = f1_score
                    best_model = model

            #acc_grid.iloc[b, e] = accs
            #auc_grid.iloc[b, e] = aucs
            f1_grid.iloc[b, e] = f1s
            #cat_acc_grid.iloc[b, e] = cat_accs


    print(f"Optimal hyperparams: {best_epochs} epochs, {best_batchsize} batchsize, {best_layers} layers. Accuracy: {best_acc}")
    return best_model, f1_grid
...
dnn_epochs = [8,9,10]
dnn_batchsizes = [225, 250, 275]
dnn_layers = [2,3,4]

#gnn_epochs = [6,7,8]
#gnn_batchsizes = [125,150, 175]

#just reduced to top 20 categories
dnn_model, dnn_f1_grid = opt_hps(DNN, red_feat_x, red_feat_y, dnn_epochs, dnn_batchsizes, dnn_layers, show_progress=True)
...
filename = 'dnn_model_red_opt_f1.pkl'
with open(filename, 'wb') as file:
    pickle.dump(dnn_model, file)

#filename = 'gnn_model_red_opt.pkl'
#with open(filename, 'wb') as file:
